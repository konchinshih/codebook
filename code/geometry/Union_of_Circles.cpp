// Dependency: circlesIntersect(), cmp(), theta()
// T = double
// identical circles: compare cross if precision is bad
auto circlesBorder(vector<Cir> c, int id) {
  vector<pair<Pt, int>> vec;
  int base = 0;
  for (int i = 0; i < sz(c); i++) if (id != i) {
    if (sgn(c[id].r - c[i].r) < 0 &&
        abs2(c[id].o - c[i].o) <=
        (c[id].r - c[i].r) * (c[id].r - c[i].r))
      base++;
    auto tmp = circlesIntersect(c[id], c[i]);
    if (sz(tmp) == 2) {
...
double circlesUnionArea(vector<Cir> c) {
  double res = 0;
  for (int i = 0; i < sz(c); i++) {
    auto seg = circlesBorder(c, i);
    auto F = [&](double t) {
      return c[i].r * (c[i].r * t +
        c[i].o.x * sin(t) - c[i].o.y * cos(t));
    };
    for (auto [l, r] : seg) {
      double tl = theta(l - c[i].o);
